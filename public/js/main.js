...
async function init() {
  try {
    const [program, schedule, tools] = await Promise.all([
      loadJSON(`${API_BASE}/api/program`),
      loadJSON(`${API_BASE}/api/schedule`),
      loadJSON(`${API_BASE}/api/tools`),
    ]);

    document.getElementById('program-cards').innerHTML =
      program.map(createCard).join('');

    document.getElementById('timeline').innerHTML =
      schedule.map(createTimelineItem).join('');
